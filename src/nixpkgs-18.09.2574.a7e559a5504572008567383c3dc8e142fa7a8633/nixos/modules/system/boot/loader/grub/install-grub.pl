...
my $subroot_path = get("subroot_path");
my $subboot_path = get("subboot_path");
my $subboot_mnt_enabled = get("subboot_mnt_enabled");
my $subboot_mnt_src = get("subboot_mnt_src");
my $subboot_mnt_dest = get("subboot_mnt_dest");
my $subboot_mnt_opt = get("subboot_mnt_opt");
...
if (stat("$bootPath")->dev != stat("$storePath")->dev) {
    $copyKernels = 1;
}
...
$grubBootDev = GrubDev($bootPath);
#my $grubBoot = GrubFs($bootPath);
#my $grubBoot = GrubFs("${subroot_path}" . $bootPath);
#my $grubStore;
#my $grubStoreDev;

if ($copyKernels == 0)
{
	$grubStoreDev = GrubDev($storePath);
	#$grubStore = GrubFs("${subroot_path}" . $storePath);
}
...
#print "\$ARGV[0][".$ARGV[0]."]\n";
#print "\$ARGV[1][".$ARGV[1]."]\n";
#print "\$bootPath["."$bootPath"."]\n";
##print "\$grubBoot->path[".($grubBoot->path)."]\n";
#print "\$grubBootDev->path[".($grubBootDev->path)."]\n";
##print "\$grubStore->path[".($grubStore->path)."]\n";
#print "\$grubStoreDev->path[".($grubStoreDev->path)."]\n";
#print "\$storePath["."${storePath}"."]\n";
#print "\$subroot_path["."${subroot_path}"."]\n";
#print "\$subboot_path["."${subboot_path}"."]\n";
...
#    my ($path) = @_;
#		#print "copyToKernelsDir()::path[$path]\n";
#    #print "copyToKernelsDir()::return(1)[".($grubStore->path) . substr($path, length("/nix/store"))."]\n" unless $copyKernels;
#    #return ($grubStore->path) . substr($path, length("/nix/store")) unless $copyKernels;
#    return ($grubStore->path) . substr($path, length("${storePath}")) unless $copyKernels;
#    #$path =~ /\/nix\/store\/(.*)/ or die;
#    $path =~ m/"${storePath}"/(.*)/ or die;
#		#if ($path)
#		#	die;
#    
#		my $name;
...
#    }
#    $copied{$dst} = 1;
#    #print "copyToKernelsDir()::return(2)[".($grubBoot->path eq "/" ? "" : $grubBoot->path) . "/kernels/$name"."]\n";
...
sub addEntry
{
    ...
    my $kernelParams;
		...
		#print "addEntry()::name,path[$name],[$path]\n";
    return unless -e "$path/kernel" && -e "$path/initrd";
    #return unless -e "$subroot_path/$path/kernel" && -e "$subroot_path/$path/initrd";
		...
		#my $kernel = copyToKernelsDir(Cwd::abs_path("$path/kernel"));
		#my $kernel = copyToKernelsDir("${subroot_path}" . Cwd::abs_path("${path}/kernel"));
		#my $kernel = "${subroot_path}" . copyToKernelsDir(Cwd::abs_path("${path}/kernel"));
		#my $kernel = "(\$drive${grubBootDev->path})" . "${subroot}" . Cwd::abs_path("${path}/kernel");
		#my $kernel = "${subroot}" . copyToKernelsDir("${subroot}" . Cwd::abs_path("${path}/kernel"));
		#my $initrd = copyToKernelsDir(Cwd::abs_path("${path}/initrd"));
		#my $initrd = copyToKernelsDir("${subroot}" . Cwd::abs_path("${path}/initrd"));
		#my $initrd = copyToKernelsDir("${subroot}" . Cwd::abs_path("${path}/initrd"));
		#my $initrd = "${subroot}" . copyToKernelsDir("${subroot}" . Cwd::abs_path("${path}/initrd"));
		...
		#print "kernel[$kernel]\n";
		#print "initrd[$initrd]\n";
    $xen = -e "$path/xen.gz" ? copyToKernelsDir(Cwd::abs_path("$path/xen.gz")) : undef;

    # FIXME: $confName
		
		#$systemConfigPath = Cwd::abs_path("$path");
		$systemConfigPath = Cwd::abs_path("$path");
		#if ("$systemConfigPath" =~ "")
		#my $systemConfigPathPrefix = "/var/store";
		
    $kernelParams = "";
    #my $kernelParams;
        #"systemConfig=" . Cwd::abs_path($path) . " " .
        #"systemConfig=" . "$storePath" . Cwd::abs_path("$path") . " ";
		#print "\$grubStore->path[".($grubStore->path)."]\n";
		#if ($systemConfigPath =~ m/\/nix\/store\/(.*)/)
		#{
		#	$kernelParams .= "systemConfig=" . "$storePath" . substr("$systemConfigPath", length("/nix/store"));
		#}
		#elsif ($systemConfigPath =~ m/\/nix\/var\/nix\/(.*)/)
		#{
		#	$kernelParams .= "systemConfig=" . "$subroot_path" . "$systemConfigPath";
		#}
		#else
		#{
			#$kernelParams .= "systemConfig=" . "${storePath}" . "${systemConfigPath}";
			$kernelParams .= "systemConfig=" . "${subroot_path}" . "${systemConfigPath}";
		#}
		
		$kernelParams .= " subroot_path=${subroot_path}";
		$kernelParams .= " subboot_path=${subboot_path}";
		$kernelParams .= " subboot_mnt_enabled=${subboot_mnt_enabled}";
		$kernelParams .= " subboot_mnt_src=${subboot_mnt_src}";
		$kernelParams .= " subboot_mnt_dest=${subboot_mnt_dest}";
		$kernelParams .= " subboot_mnt_opt=${subboot_mnt_opt}";
		...
		$xenParams = $xen && -e "$path/xen-params" ? readFile("$path/xen-params") : "";

    if ($grubVersion == 1) {
        $conf .= "title $name\n";
        $conf .= "  $extraPerEntryConfig\n" if $extraPerEntryConfig;
        $conf .= "  kernel $xen $xenParams\n" if $xen;
				$conf .= "  echo 'Loading kernel...'\n";
        $conf .= "  " . ($xen ? "module" : "kernel") . " $kernel $kernelParams\n";
				$conf .= "  echo 'Loading initrd...'\n";
        $conf .= "  " . ($xen ? "module" : "initrd") . " $initrd\n\n";
    } else {
        $conf .= "menuentry \"$name\" {\n";
        $conf .= $grubBootDev->search . "\n";
        if ($copyKernels == 0) {
            $conf .= $grubStoreDev->search . "\n";
        }
        if ($extraInitrd) {
...
#addEntry("NixOS - Default", "${subroot_path}" . "${defaultConfig}");
...
sub addProfile {
    ...
    foreach my $link (@links) {
        ...
        addEntry(nrFromGen($link) . ". NixOS " . "$version ($date)", $link);
    }

    $conf .= "}\n" if $grubVersion == 2;
}

addProfile ("${subroot_path}" . "/nix/var/nix/profiles/system", "NixOS - All configurations");
